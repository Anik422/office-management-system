...
class Credit_win:
    def __init__(self, root):
        ...
        credit_lbl_title = Label(self.root, text="ADD CREDIT DATA DETAILS", font=("times new roman",20,"bold"), bg="#03396c", fg="#eeeeee", bd=2, relief="groove")
    ...
    #   ========================  variables  ========================
        ...
        lable_frame_laft = LabelFrame(self.root, text="Credit Detatls", bd=2, relief="groove", font=("times new roman",20,"bold"), padx=2)
    ...
    def table_data(self, view_table_name, date):
        ...
        self.data_deatlis.column('amount', width=10)
    ...
    def add_data(self):
        if self.description.get() == "":
            messagebox.showerror("Error", "Description fields are requaired", parent=self.root)
        elif self.amount.get() <= 0:
            messagebox.showerror("Error", "Amount fields are requaired", parent=self.root)
            ...
            try:
                ...
                t_name = str(self.table_name.get())
                amount = self.amount.get()
                db.table_data_entry(dt, des, amount, "C", t_name)
    ...
    #   ========================  add table data view data ========================
    def add_table_data_view(self, table_name):
        table_value = db.add_table_data_fetch(table_name, "C")
    ...
    def search_table_view(self):
        ...
        if table_name == "Select Table":
            ...
            table_value = db.search_table_fetch(table_name, "C", search_date)
            if len(table_value) == 0:
    ...
    def search_table_all_data_view(self):
        ...
        if table_name == "Select Table":
            ...
            table_value = db.search_table_all_data_fetch(table_name, "C",)
            if len(table_value) == 0:
                messagebox.showerror("Error", "Table data nai", parent=self.root)
            else:
                try:
